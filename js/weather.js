    ...
    .then((data) => {
      for (let i = 0; i < 4; i++) {
        const dailyWeatherIcon = document.querySelector(
          `.weather__daily.row${i} .weather__icon`
        );
        const dailyWeek = document.querySelector(
          `.weather__daily.row${i} .weather__day`
        );
        const dailyTempMax = document.querySelector(
          `.weather__daily.row${i} .weather__temp-max`
        );
        const dailyTempMin = document.querySelector(
          `.weather__daily.row${i} .weather__temp-min`
        );
        if (dailyWeatherIcon) {
